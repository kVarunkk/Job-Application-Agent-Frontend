...
import { FavoritedJob } from "@/app/api/updates/applicants/favorites/route";

interface FavoriteJobReminderEmailProps {
  userName: string;
  favoritedJobs: FavoritedJob[];
}
...
export const FavoriteJobReminderEmail = ({
  userName,
  favoritedJobs,
}: FavoriteJobReminderEmailProps) => {
  return (
    <BaseEmailLayout previewText={""}>
      ...
      <Text className="text-base text-gray-700 mb-6 ">
        You saved{" "}
        <strong>
          {favoritedJobs.length} job{favoritedJobs.length > 1 ? "s" : ""}
        </strong>{" "}
        last week, and now is a great time to Apply to these jobs!
      </Text>
      ...
      <Section className="text-start my-16">
        <Button
          href="https://gethired.devhub.co.in/jobs?tab=saved&sortBy=created_at&sortOrder=desc"
...
export default FavoriteJobReminderEmail;
